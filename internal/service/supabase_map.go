...
func mapEvent(m map[string]any) model.Event {
	return model.Event{
		...
		GcashHandle:                asStrPtr(m, "gcash_handle"),
		VenmoHandle:                asStrPtr(m, "venmo_handle"),
		ClubID:                     asStrPtr(m, "club_id"),
		VenueNotes:                 asStrPtr(m, "venue_notes"),
		WaiverURL:                  asStrPtr(m, "waiver_url"),
		ConfirmEmailSubject:        asStrPtr(m, "confirm_email_subject"),
		ConfirmEmailMessage:        asStrPtr(m, "confirm_email_message"),
		...
		AddonTeeName:               asStr(m, "addon_tee_name"),
		AddonTeeFrontURL:           asStr(m, "addon_tee_front_url"),
		AddonTeeBackURL:            asStr(m, "addon_tee_back_url"),
		AddonTeeSizes:              asStrSlice(m, "addon_tee_sizes"),
...
func mapBracket(m map[string]any) model.Bracket {
	return model.Bracket{
		...
		StartMinutes:   asIntPtr(m, "start_minutes"),
		TeamCount:      asIntPtr(m, "team_count"),
		PlayersPerTeam: asIntPtr(m, "players_per_team"),
	}
}
...
const matchSelect = "id,bracket_id,stage,bracket_tier,bracket_group,bracket_round,bracket_slot," +
	"team1_score,team2_score,winning_team,live_team1,live_team2,games,status,result_type,counts_for_diff,play_order,duration_minutes,scheduled_day,completed_at,line_type," +
	"court:courts!court_id(court_number)," +
	"round:rounds!round_id(id,round_number,status,started_at,created_at)," +
...
func mapMatch(m map[string]any) model.Match {
	mt := model.Match{
		...
		// Null/absent → true (normal + legacy matches count); only an explicit
		// false (fabricated forfeit/walkover/retire-without-score) is excluded.
		CountsForDiff:   m["counts_for_diff"] == nil || asBool(m, "counts_for_diff"),
	...
	if r := asMap(m, "round"); r != nil {
		...
		mt.RoundCreatedAt = asStrPtr(r, "created_at")
	}
	return mt
}
